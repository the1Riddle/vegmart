	...
	//}else if( f.phone.value == '' ){
		...
		notice( f.message );
		 ...
		 jQuery.ajax({
			url: 'mail.php',
			type: 'post',
			data: jQuery('form#vegmart-contact').serialize(),
			complete: function(data) {
				jQuery('#form_status').html(data.responseText);
				jQuery('#vegmart-contact').find('input,textarea').attr({value:''});
				jQuery('#vegmart-contact').css({opacity:1});
				jQuery('#vegmart-contact').remove();
			}
		});
		jQuery('#form_status').html('<span class="loading">Sending your message...</span>');
		jQuery('#vegmart-contact').animate({opacity:0.3});
		jQuery('#vegmart-contact').find('input,textarea,button').css('border','none').attr({'disabled':''});
	}
...
function notice( f ){
	jQuery('#vegmart-contact').find('input,textarea').css('border','none');
